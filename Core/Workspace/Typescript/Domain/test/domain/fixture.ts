...
export function securityResponse(m: Meta): SecurityResponse {
  ...
  return {
    ...
    permissions: [
      ['901', m.Organisation.id, m.Organisation.JustDoIt.id, Operations.Execute.toString()],
      ['902', m.Organisation.id, m.Organisation.Name.id, Operations.Read.toString()],
      ['903', m.Organisation.id, m.Organisation.Owner.id, Operations.Write.toString()],
    ]
  };
}
