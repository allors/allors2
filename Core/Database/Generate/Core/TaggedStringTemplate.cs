...
namespace Allors.Development.Repository.Generation
{
    ...
    public class TaggedStringTemplate
    {
        ...
        internal TaggedStringTemplate(FileInfo fileInfo)
        {
        ...
        internal void Generate(TaggedMetaPopulation metaPopulation, DirectoryInfo outputDirectory, string output, Log log)
        {
            ...
            try
            {
                ...
                foreach (XmlElement generation in configurationXml.DocumentElement.SelectNodes(GenerationKey))
                {
                    ...
                    if (generation.HasAttribute(InputKey))
                    {
                        ...
                        if (metaPopulation.Find(input) is TaggedObjectType objectType)
                        {
                            template.Add(ObjectTypeKey, objectType);
                        }
                        else
                        {
                            if (metaPopulation.Find(input) is TaggedRelationType relationType)
                            {
