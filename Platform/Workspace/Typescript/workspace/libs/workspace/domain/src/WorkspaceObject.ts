import { ObjectType, MetaPopulation, RoleType } from '@allors/workspace/meta';
import { Compressor, SyncResponseObject } from '@allors/workspace/protocol';
...
export class WorkspaceObject implements IWorkspaceObject {
  ...
  constructor(
    ...
    if (objectTypeOrMetaPopulation instanceof ObjectType) {
      ...
      this.objectType = metaPopulation.metaObjectById.get(
        syncResponseObject.t
      ) as ObjectType;
      ...
      this.sortedAccessControlIds = syncResponseObject?.a
        ? sortedAccessControlIdsDecompress!(syncResponseObject.a)
        : undefined;
      this.sortedDeniedPermissionIds = syncResponseObject?.d
        ? sortedDeniedPermissionIdsDecompress!(syncResponseObject.d)
        : undefined;
    }
  }
  isPermitted(permission: Permission): boolean {
    ...
    if (
      this.sortedDeniedPermissionIds !== this.cachedSortedDeniedPermissionIds
    ) {
    ...
    if (
      this.cachedDeniedPermissions &&
      this.cachedDeniedPermissions.has(permission)
    ) {
      return false;
    }
