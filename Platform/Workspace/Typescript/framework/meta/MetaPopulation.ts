    ...
    constructor(data: MetaData) {
        ...
        this.composites.forEach((objectType) => {
            ...
            objectType.subtypes.forEach((subtype) => {
                subtype.roleTypes.filter((subtypeRoleType) => !objectType.roleTypes.find((v) => v.id === subtypeRoleType.id)).forEach((v) => objectType.roleTypeByName.set(`${subtype.name}_${v.name}`, v));
