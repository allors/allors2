...
export class WorkspaceObject implements IWorkspaceObject {
    ...
    sync(syncResponseObject: SyncResponseObject, sortedAccessControlIdsDecompress: (compressed: string) => string, sortedDeniedPermissionIdsDecompress: (compressed: string) => string, metaPopulation: MetaPopulation) {
    ...
    isPermitted(permission: Permission): boolean {
        ...
        if (this.sortedAccessControlIds !== this.cachedSortedAccessControlIds) {
            ...
            if (this.sortedAccessControlIds) {
                this.cachedAccessControls = this.sortedAccessControlIds.split(Compressor.itemSeparator).map((v) => this.workspace.accessControlById.get(v));
            } else {
