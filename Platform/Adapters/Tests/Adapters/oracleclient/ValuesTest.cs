...
namespace Allors.Database.Special.OracleClient
{
    ...
    [TestFixture]
    public abstract class ValuesTest : Special.UnitTest
    {
        DateTime MAXIMUM_DATETIME
        {
            get { return new DateTime(9999, 12, 31, 11, 59, 00, 00); }
        }

        DateTime MINIMUM_DATETIME
        {
        ...
        public override void AllorsDate()
        {
            ...
            {
                C1 values = C1.Create(this.Session);
                values.C1AllorsDate = MINIMUM_DATETIME;
                values.I1AllorsDate = MINIMUM_DATETIME;
                Assert.AreEqual(MINIMUM_DATETIME.Date, values.C1AllorsDate);
                Assert.AreEqual(MINIMUM_DATETIME.Date, values.I1AllorsDate);
            }

            // Maximum
            {
                C1 values = C1.Create(this.Session);
                values.C1AllorsDate = MAXIMUM_DATETIME;
                values.I1AllorsDate = MAXIMUM_DATETIME;
                Assert.AreEqual(MAXIMUM_DATETIME.Date, values.C1AllorsDate);
                Assert.AreEqual(MAXIMUM_DATETIME.Date, values.I1AllorsDate);
            }
