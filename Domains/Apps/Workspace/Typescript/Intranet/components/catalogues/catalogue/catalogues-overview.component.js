...
let CataloguesOverviewComponent = class CataloguesOverviewComponent {
    constructor(workspaceService, errorService, formBuilder, titleService, snackBar, router, dialogService, media, changeDetectorRef) {
        ...
        this.subscription = combined$
            ...
            if (data.name) {
                ...
                predicates.push(new framework_1.Like({ roleType: m.Catalogue.Name, value: like }));
            }
            const query = [new framework_1.Query({
                    ...
                    predicate,
                    include: [
                        new framework_1.TreeNode({ roleType: m.Catalogue.CatalogueImage }),
                        new framework_1.TreeNode({ roleType: m.Catalogue.ProductCategories }),
                    ],
                })];
            return this.scope.load("Pull", new framework_1.PullRequest({ query }));
        })
            .subscribe((loaded) => {
            this.data = loaded.collections.catalogues;
...
CataloguesOverviewComponent = __decorate([
    core_1.Component({
        template: `
<mat-toolbar>
...
exports.CataloguesOverviewComponent = CataloguesOverviewComponent;
