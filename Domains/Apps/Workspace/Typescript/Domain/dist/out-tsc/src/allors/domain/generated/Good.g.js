"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
// Allors generated file.
// Do not edit this file, changes will be overwritten.
/* tslint:disable */
const framework_1 = require("../../framework");
class Good extends framework_1.SessionObject {
    get CanReadQuantityOnHand() {
        return this.canRead('QuantityOnHand');
    }
    get QuantityOnHand() {
        return this.get('QuantityOnHand');
    }
    get CanReadAvailableToPromise() {
        return this.canRead('AvailableToPromise');
    }
    get AvailableToPromise() {
        return this.get('AvailableToPromise');
    }
    get CanReadInventoryItemKind() {
        return this.canRead('InventoryItemKind');
    }
    get CanWriteInventoryItemKind() {
        return this.canWrite('InventoryItemKind');
    }
    get InventoryItemKind() {
        return this.get('InventoryItemKind');
    }
    set InventoryItemKind(value) {
        this.set('InventoryItemKind', value);
    }
    get CanReadBarCode() {
        return this.canRead('BarCode');
    }
    get CanWriteBarCode() {
        return this.canWrite('BarCode');
    }
    get BarCode() {
        return this.get('BarCode');
    }
    set BarCode(value) {
        this.set('BarCode', value);
    }
    get CanReadFinishedGood() {
        return this.canRead('FinishedGood');
    }
    get CanWriteFinishedGood() {
        return this.canWrite('FinishedGood');
    }
    get FinishedGood() {
        return this.get('FinishedGood');
    }
    set FinishedGood(value) {
        this.set('FinishedGood', value);
    }
    get CanReadSku() {
        return this.canRead('Sku');
    }
    get CanWriteSku() {
        return this.canWrite('Sku');
    }
    get Sku() {
        return this.get('Sku');
    }
    set Sku(value) {
        this.set('Sku', value);
    }
    get CanReadArticleNumber() {
        return this.canRead('ArticleNumber');
    }
    get CanWriteArticleNumber() {
        return this.canWrite('ArticleNumber');
    }
    get ArticleNumber() {
        return this.get('ArticleNumber');
    }
    set ArticleNumber(value) {
        this.set('ArticleNumber', value);
    }
    get CanReadManufacturedBy() {
        return this.canRead('ManufacturedBy');
    }
    get CanWriteManufacturedBy() {
        return this.canWrite('ManufacturedBy');
    }
    get ManufacturedBy() {
        return this.get('ManufacturedBy');
    }
    set ManufacturedBy(value) {
        this.set('ManufacturedBy', value);
    }
    get CanReadManufacturerId() {
        return this.canRead('ManufacturerId');
    }
    get CanWriteManufacturerId() {
        return this.canWrite('ManufacturerId');
    }
    get ManufacturerId() {
        return this.get('ManufacturerId');
    }
    set ManufacturerId(value) {
        this.set('ManufacturerId', value);
    }
    get CanReadSuppliedBy() {
        return this.canRead('SuppliedBy');
    }
    get CanWriteSuppliedBy() {
        return this.canWrite('SuppliedBy');
    }
    get SuppliedBy() {
        return this.get('SuppliedBy');
    }
    set SuppliedBy(value) {
        this.set('SuppliedBy', value);
    }
    get CanReadProductSubstitutions() {
        return this.canRead('ProductSubstitutions');
    }
    get CanWriteProductSubstitutions() {
        return this.canWrite('ProductSubstitutions');
    }
    get ProductSubstitutions() {
        return this.get('ProductSubstitutions');
    }
    AddProductSubstitution(value) {
        return this.add('ProductSubstitutions', value);
    }
    RemoveProductSubstitution(value) {
        return this.remove('ProductSubstitutions', value);
    }
    set ProductSubstitutions(value) {
        this.set('ProductSubstitutions', value);
    }
    get CanReadProductIncompatibilities() {
        return this.canRead('ProductIncompatibilities');
    }
    get CanWriteProductIncompatibilities() {
        return this.canWrite('ProductIncompatibilities');
    }
    get ProductIncompatibilities() {
        return this.get('ProductIncompatibilities');
    }
    AddProductIncompatibility(value) {
        return this.add('ProductIncompatibilities', value);
    }
    RemoveProductIncompatibility(value) {
        return this.remove('ProductIncompatibilities', value);
    }
    set ProductIncompatibilities(value) {
        this.set('ProductIncompatibilities', value);
    }
    get CanReadPrimaryPhoto() {
        return this.canRead('PrimaryPhoto');
    }
    get CanWritePrimaryPhoto() {
        return this.canWrite('PrimaryPhoto');
    }
    get PrimaryPhoto() {
        return this.get('PrimaryPhoto');
    }
    set PrimaryPhoto(value) {
        this.set('PrimaryPhoto', value);
    }
    get CanReadPhotos() {
        return this.canRead('Photos');
    }
    get CanWritePhotos() {
        return this.canWrite('Photos');
    }
    get Photos() {
        return this.get('Photos');
    }
    AddPhoto(value) {
        return this.add('Photos', value);
    }
    RemovePhoto(value) {
        return this.remove('Photos', value);
    }
    set Photos(value) {
        this.set('Photos', value);
    }
    get CanReadKeywords() {
        return this.canRead('Keywords');
    }
    get CanWriteKeywords() {
        return this.canWrite('Keywords');
    }
    get Keywords() {
        return this.get('Keywords');
    }
    set Keywords(value) {
        this.set('Keywords', value);
    }
    get CanReadInternalComment() {
        return this.canRead('InternalComment');
    }
    get CanWriteInternalComment() {
        return this.canWrite('InternalComment');
    }
    get InternalComment() {
        return this.get('InternalComment');
    }
    set InternalComment(value) {
        this.set('InternalComment', value);
    }
    get CanReadPrimaryProductCategory() {
        return this.canRead('PrimaryProductCategory');
    }
    get CanWritePrimaryProductCategory() {
        return this.canWrite('PrimaryProductCategory');
    }
    get PrimaryProductCategory() {
        return this.get('PrimaryProductCategory');
    }
    set PrimaryProductCategory(value) {
        this.set('PrimaryProductCategory', value);
    }
    get CanReadSupportDiscontinuationDate() {
        return this.canRead('SupportDiscontinuationDate');
    }
    get CanWriteSupportDiscontinuationDate() {
        return this.canWrite('SupportDiscontinuationDate');
    }
    get SupportDiscontinuationDate() {
        return this.get('SupportDiscontinuationDate');
    }
    set SupportDiscontinuationDate(value) {
        this.set('SupportDiscontinuationDate', value);
    }
    get CanReadSalesDiscontinuationDate() {
        return this.canRead('SalesDiscontinuationDate');
    }
    get CanWriteSalesDiscontinuationDate() {
        return this.canWrite('SalesDiscontinuationDate');
    }
    get SalesDiscontinuationDate() {
        return this.get('SalesDiscontinuationDate');
    }
    set SalesDiscontinuationDate(value) {
        this.set('SalesDiscontinuationDate', value);
    }
    get CanReadLocalisedNames() {
        return this.canRead('LocalisedNames');
    }
    get CanWriteLocalisedNames() {
        return this.canWrite('LocalisedNames');
    }
    get LocalisedNames() {
        return this.get('LocalisedNames');
    }
    AddLocalisedName(value) {
        return this.add('LocalisedNames', value);
    }
    RemoveLocalisedName(value) {
        return this.remove('LocalisedNames', value);
    }
    set LocalisedNames(value) {
        this.set('LocalisedNames', value);
    }
    get CanReadLocalisedDescriptions() {
        return this.canRead('LocalisedDescriptions');
    }
    get CanWriteLocalisedDescriptions() {
        return this.canWrite('LocalisedDescriptions');
    }
    get LocalisedDescriptions() {
        return this.get('LocalisedDescriptions');
    }
    AddLocalisedDescription(value) {
        return this.add('LocalisedDescriptions', value);
    }
    RemoveLocalisedDescription(value) {
        return this.remove('LocalisedDescriptions', value);
    }
    set LocalisedDescriptions(value) {
        this.set('LocalisedDescriptions', value);
    }
    get CanReadLocalisedComments() {
        return this.canRead('LocalisedComments');
    }
    get CanWriteLocalisedComments() {
        return this.canWrite('LocalisedComments');
    }
    get LocalisedComments() {
        return this.get('LocalisedComments');
    }
    AddLocalisedComment(value) {
        return this.add('LocalisedComments', value);
    }
    RemoveLocalisedComment(value) {
        return this.remove('LocalisedComments', value);
    }
    set LocalisedComments(value) {
        this.set('LocalisedComments', value);
    }
    get CanReadDescription() {
        return this.canRead('Description');
    }
    get Description() {
        return this.get('Description');
    }
    get CanReadProductComplement() {
        return this.canRead('ProductComplement');
    }
    get CanWriteProductComplement() {
        return this.canWrite('ProductComplement');
    }
    get ProductComplement() {
        return this.get('ProductComplement');
    }
    set ProductComplement(value) {
        this.set('ProductComplement', value);
    }
    get CanReadOptionalFeatures() {
        return this.canRead('OptionalFeatures');
    }
    get CanWriteOptionalFeatures() {
        return this.canWrite('OptionalFeatures');
    }
    get OptionalFeatures() {
        return this.get('OptionalFeatures');
    }
    AddOptionalFeature(value) {
        return this.add('OptionalFeatures', value);
    }
    RemoveOptionalFeature(value) {
        return this.remove('OptionalFeatures', value);
    }
    set OptionalFeatures(value) {
        this.set('OptionalFeatures', value);
    }
    get CanReadVariants() {
        return this.canRead('Variants');
    }
    get CanWriteVariants() {
        return this.canWrite('Variants');
    }
    get Variants() {
        return this.get('Variants');
    }
    AddVariant(value) {
        return this.add('Variants', value);
    }
    RemoveVariant(value) {
        return this.remove('Variants', value);
    }
    set Variants(value) {
        this.set('Variants', value);
    }
    get CanReadName() {
        return this.canRead('Name');
    }
    get Name() {
        return this.get('Name');
    }
    get CanReadIntroductionDate() {
        return this.canRead('IntroductionDate');
    }
    get CanWriteIntroductionDate() {
        return this.canWrite('IntroductionDate');
    }
    get IntroductionDate() {
        return this.get('IntroductionDate');
    }
    set IntroductionDate(value) {
        this.set('IntroductionDate', value);
    }
    get CanReadDocuments() {
        return this.canRead('Documents');
    }
    get CanWriteDocuments() {
        return this.canWrite('Documents');
    }
    get Documents() {
        return this.get('Documents');
    }
    AddDocument(value) {
        return this.add('Documents', value);
    }
    RemoveDocument(value) {
        return this.remove('Documents', value);
    }
    set Documents(value) {
        this.set('Documents', value);
    }
    get CanReadStandardFeatures() {
        return this.canRead('StandardFeatures');
    }
    get CanWriteStandardFeatures() {
        return this.canWrite('StandardFeatures');
    }
    get StandardFeatures() {
        return this.get('StandardFeatures');
    }
    AddStandardFeature(value) {
        return this.add('StandardFeatures', value);
    }
    RemoveStandardFeature(value) {
        return this.remove('StandardFeatures', value);
    }
    set StandardFeatures(value) {
        this.set('StandardFeatures', value);
    }
    get CanReadUnitOfMeasure() {
        return this.canRead('UnitOfMeasure');
    }
    get CanWriteUnitOfMeasure() {
        return this.canWrite('UnitOfMeasure');
    }
    get UnitOfMeasure() {
        return this.get('UnitOfMeasure');
    }
    set UnitOfMeasure(value) {
        this.set('UnitOfMeasure', value);
    }
    get CanReadEstimatedProductCosts() {
        return this.canRead('EstimatedProductCosts');
    }
    get CanWriteEstimatedProductCosts() {
        return this.canWrite('EstimatedProductCosts');
    }
    get EstimatedProductCosts() {
        return this.get('EstimatedProductCosts');
    }
    AddEstimatedProductCost(value) {
        return this.add('EstimatedProductCosts', value);
    }
    RemoveEstimatedProductCost(value) {
        return this.remove('EstimatedProductCosts', value);
    }
    set EstimatedProductCosts(value) {
        this.set('EstimatedProductCosts', value);
    }
    get CanReadProductObsolescences() {
        return this.canRead('ProductObsolescences');
    }
    get CanWriteProductObsolescences() {
        return this.canWrite('ProductObsolescences');
    }
    get ProductObsolescences() {
        return this.get('ProductObsolescences');
    }
    AddProductObsolescence(value) {
        return this.add('ProductObsolescences', value);
    }
    RemoveProductObsolescence(value) {
        return this.remove('ProductObsolescences', value);
    }
    set ProductObsolescences(value) {
        this.set('ProductObsolescences', value);
    }
    get CanReadSelectableFeatures() {
        return this.canRead('SelectableFeatures');
    }
    get CanWriteSelectableFeatures() {
        return this.canWrite('SelectableFeatures');
    }
    get SelectableFeatures() {
        return this.get('SelectableFeatures');
    }
    AddSelectableFeature(value) {
        return this.add('SelectableFeatures', value);
    }
    RemoveSelectableFeature(value) {
        return this.remove('SelectableFeatures', value);
    }
    set SelectableFeatures(value) {
        this.set('SelectableFeatures', value);
    }
    get CanReadVatRate() {
        return this.canRead('VatRate');
    }
    get CanWriteVatRate() {
        return this.canWrite('VatRate');
    }
    get VatRate() {
        return this.get('VatRate');
    }
    set VatRate(value) {
        this.set('VatRate', value);
    }
    get CanReadBasePrices() {
        return this.canRead('BasePrices');
    }
    get BasePrices() {
        return this.get('BasePrices');
    }
    get CanReadProductCategories() {
        return this.canRead('ProductCategories');
    }
    get CanWriteProductCategories() {
        return this.canWrite('ProductCategories');
    }
    get ProductCategories() {
        return this.get('ProductCategories');
    }
    AddProductCategory(value) {
        return this.add('ProductCategories', value);
    }
    RemoveProductCategory(value) {
        return this.remove('ProductCategories', value);
    }
    set ProductCategories(value) {
        this.set('ProductCategories', value);
    }
    get CanReadComment() {
        return this.canRead('Comment');
    }
    get CanWriteComment() {
        return this.canWrite('Comment');
    }
    get Comment() {
        return this.get('Comment');
    }
    set Comment(value) {
        this.set('Comment', value);
    }
    get CanReadUniqueId() {
        return this.canRead('UniqueId');
    }
    get CanWriteUniqueId() {
        return this.canWrite('UniqueId');
    }
    get UniqueId() {
        return this.get('UniqueId');
    }
    set UniqueId(value) {
        this.set('UniqueId', value);
    }
    get CanExecuteDelete() {
        return this.canExecute('Delete');
    }
    get Delete() {
        return new framework_1.Method(this, 'Delete');
    }
}
exports.Good = Good;
//# sourceMappingURL=Good.g.js.map