...
namespace Allors.Document.OpenDocument
{
    ...
    public class OpenDocumentTemplate
    {
                    ...
                    foreach (var entry in archive.Entries)
                    {
                        using (var memoryStream = new MemoryStream())
                        {
                            using (var zipStream = entry.Open())
                            {
                                ...
                                {
                                    var content = new OpenDocumentTemplateContent(file, this.leftDelimiter, this.rightDelimiter);
                                    this.stringTemplate = content.ToStringTemplate();
                                }
        ...
        {
            var render = new OpenDocumentRendering(model, this.manifest, imageByName, this.stringTemplate, this.leftDelimiter, this.rightDelimiter, this.fileByFileName);
    ...
    public class OpenDocumentTemplate<T> : OpenDocumentTemplate
    {
        ...
        public OpenDocumentTemplate(byte[] document, char leftDelimiter = DefaultLeftDelimiter, char rightDelimiter = DefaultRightDelimiter)
            : base(document, leftDelimiter, rightDelimiter)
        {
